...
kp_vals = []
for exp in range(19, 41):  # 10^4 to 10^40
    for mul in range(1, 10):  # 1*10^exp, 2*10^exp, ... 9*10^exp
        kp_vals.append(mul * 10**exp)
kp_vals = np.array(kp_vals)

# Write header to results file
header = f'{"kp gain":>9} {"kd gain":>9} {"mean":>12} {"median":>12} {"min":>9} {"max":>12}\n'
...
for i in range(len(kp_vals)):
    ...
    kp_sci = short_sci(kp)
    kd_sci = short_sci(kd)
    line = f"{kp_sci:>9} {kd_sci:>9} {mean_val:12.4f} {median_val:12.4f} {min_val:9.4f} {max_val:12.4f}\n"
